...
node* xoasv(node* root)
{
	...
	while (cur!=NULL)
	{
		if (strcmp((cur->data).mssv,str)==0)
		{
			if (cur->next==NULL && cur->prev==NULL) root=NULL; else
			if (cur->next==NULL) (cur->prev)->next=NULL; else
			if (cur->prev==NULL) 
			{
				root=cur->next;
				root->prev=NULL;
...
void testdslkdoi(node* root)
{
	printf("\n***Danh sach sinh vien(In nguoc lai)***\n");
	printf("%-10s%-20s%-10s\n","MSSV","Ho va ten","Diem");
...
int main()
{
	...
	int c=choncachnhap();
	if (c==1) root=nhapdulieu(root); else root=doctep(root);

	if (root==NULL)
	{
		printf("!!!Danh sach rong - Chuong trinh ket thuc");
		return 0;
	}
