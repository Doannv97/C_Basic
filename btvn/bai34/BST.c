...
node* makeNode(keyType key[], char password[]){
...
int insertNode(BSTree* Root, keyType key[], char password[]){
	if (*Root==NULL) *Root=makeNode(key,password);
	else if (stringcmp(key,(*Root)->key)==-1) insertNode(&((*Root)->left),key,password);
	else if (stringcmp(key,(*Root)->key)==1) insertNode(&((*Root)->right),key,password);
	else return 0;
	return 1;
}
